...
num_lstm = 512  # np.random.randint(175, 275)
...
STAMP = 'lstm_%d_%d_%.2f_%.2f' % \
    (num_lstm, num_dense, rate_drop_lstm, rate_drop_dense)
...
for word, i in word_index.items():
    if word in word2vec.vocab:
        ...
        embedding_matrix[i] = [(random() - 0.5) * 0.01 for _ in range(300)]
...
lstm_layer2 = LSTM(num_lstm, dropout=rate_drop_lstm,
                   recurrent_dropout=rate_drop_lstm)

sequence_1_input = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='int32')
embedded_sequences_1 = embedding_layer(sequence_1_input)
x1 = lstm_layer2(embedded_sequences_1)

sequence_2_input = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='int32')
embedded_sequences_2 = embedding_layer(sequence_2_input)
y1 = lstm_layer2(embedded_sequences_2)
...
submission.to_csv('%.4f_' % (bst_val_score) + STAMP + '.csv',
                  index=False, columns=['test_id', 'is_duplicate'])
...
#         preds.append(max(p1, p2))
#     elif p1 < 0.4 and p2 < 0.4:
#         preds.append(min(p1, p2))
#     else:
